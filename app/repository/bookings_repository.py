...
class BookingsRepository:
    """
    Класс BookingsRepository используется для обработки операций с базой данных, связанных с бронированием отелей.
    """

    @classmethod
    async def create_booking(cls, booking_data: BookingCreate, session: SessionDependency) -> int:
    ...
    @classmethod
    async def get_bookings(cls, session: SessionDependency) -> list[BookingResponse]:
    ...
    @classmethod
    async def cancel_booking(cls, booking_id: int, session: SessionDependency) -> dict:
